...
import { agentLogin, buyerLogin } from "../../api";
...
const AgentSignIn = () => {
    ...
    const handleLogin = async (data) => {
        try {
            ...
            loader.startLoader();
            await agentLogin(data).then(response => {
                // if (!response.success) throw Error(response.message);
                console.log(response);
                auth.login({...response, token: response.accessToken }, USER_TYPES.AGENT);
                toast('Login Successful', toastSuccess);
                setTimeout(() => {
                ...
                <div className="w-full flex flex-col items-center gap-4 lg:gap-5 text-[#000]">
                    ...
                    <div className="flex flex-col gap-5 md:gap-5 lg:gap-4 macScreens:gap-10 w-full">
                        ...
                        <div className="flex flex-col gap-2 md:gap-5 lg:gap-2 macScreens:gap-2">
                            <CustomButton className="my-custom-button" text="Sign In" onClick={() => handleLogin(login)} disabled={loader.loader} />
                        </div>
                    </div>
                </div>
            </div>

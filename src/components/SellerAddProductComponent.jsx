...
const SellAddProductComponent = () => {
    ...
    const handleProductSubmit = async (data) => {
        ...
        let isAllDataChecked = validateProductData(data.productData, {
...
specifications:productData.specifications
        ...
        // await uploadImages({ images: productData.productMedia }, { Authorization: `Bearer ${auth.user.token}` }).then(async response => {
        //     if (!response.success) throw Error(response.message);
        //     toast(response.message, toastSuccess);

        //     let urls = response.data.urls;
        //     let productSpecifications = data.productData.specifications.map(specification => {
        //         let { id, ...rest } = specification;
        //         return rest;
        //     });
        ...
        //         delete productData.productSubCategory;
        //     }

        //     await sellerCreateProduct(productData, { Authorization: `Bearer ${auth.user.token}` }).then(response => {
        //         if (!response.success) throw Error(response.message);
        //         toast(response.message, toastSuccess);
        //         setTimeout(() => {
        //             navigate(-1);
        ...
        console.log(productData, processedImages);

        console.log(`the specifications posted ======
    ...
    return ( 
        <div className="flex flex-col gap-4 h-full">
            <div className="flex gap-2 items-center">
                ...
                <h1 className="font-semibold text-[1.5rem] text-[#000]">Add Product</h1>
            </div>
                        ...
                        <div className="w-full flex flex-col gap-5">
                            <div className="grid grid-cols-2 gap-x-20 w-full">
                                <div className="input-box flex flex-col gap-1 lg:gap-2 macScreens:gap-2  macScreens:text-[1rem] lg:text-[.8rem]">
                                    <InputField label={<p className="text-[#161616]">Product Category</p>} name="productCategory" type="select" options={category} required selected={(selectedData) => {
                            ...
                            <div className="input-box flex flex-col gap-1 lg:gap-2 macScreens:gap-2  macScreens:text-[1rem] lg:text-[.8rem]">
                                <InputField label="Product Name" name="name" placeholder="Enter product name..." type="text" onChange={(e) => handleProductData(e)} />
                            </div>
                            <div className="input-box flex flex-col gap-1 lg:gap-2 macScreens:gap-2  macScreens:text-[1rem] lg:text-[.8rem]">
                                <label className="text-[.6rem] md:text-[1rem]">Short Description</label>
                                <textarea name="shortDescription" placeholder="Write a short description..." className="rounded-[10px] border-[1px] border-[#848484] p-5" id="" rows="5" onChange={(e) => handleProductData(e)}></textarea>
                            </div>
                            <div className="input-box flex flex-col gap-1 lg:gap-2 macScreens:gap-2  macScreens:text-[1rem] lg:text-[.8rem]">
                                <label className="text-[.6rem] md:text-[1rem]">Long Description</label>
                                <WYSIWYG getContent={(typedData) => setLongDescription(typedData)}/>
                            </div>
                            <div className="input-box flex flex-col gap-1 lg:gap-2 macScreens:gap-2  macScreens:text-[1rem] lg:text-[.8rem]">
                                <InputField label={`Product Price: ${formattedPrice}`} name="price" placeholder="Enter product price" type="text" onChange={(e) => {
                            ...
                            <div className="h-32 flex justify-end items-center">
                                <CustomButton buttonText="Publish Product" btnClassName="bg-[#00753E] text-[#FFF] rounded-[10px] border-[1px] border-[#00753E] lg:rounded-[10px] flex justify-center items-center lg:py-2 lg:-mb-2" onClick={() => handleProductSubmit({ productMedia, productData })}/>
                            </div>
                        </div>
                    </div>
                </div>
...
export default SellAddProductComponent;
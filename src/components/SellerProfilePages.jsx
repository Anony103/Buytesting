...
import SellerBusinessDisplay  from "../components/SellerBusinessDisplay";
...
import { getProductsBySellerId } from "../api";
...
const SellerProfilePages = () => {
  ...
  const fetchSellerProductData = async () => {
    app.startLoader();
        try {
            const response = await getProductsBySellerId(id);
  ...
  // Ensure that user exists before destructuring
  const user = products.length > 0 ? products[0].user : {};
  const { businessName, avatar, address, createdAt, email, facebook, instagram, isVerified, phoneNumber, twitter, whatsapp } = user || {};
  ...
  return (
    <section className="overflow-x-hidden w-full flex flex-col items-center macScreens:gap-5 lg:gap-3 gap-2 pb-[3rem]">
      ...
      <div className="relative md:w-[90%] w-[95%] flex flex-col justify-center items-center gap-10 mt-[7rem]">
        <BuyerPathBreadCrumb home="Seller's List" shop=">  Seller's Profile"/>

        <SellerBusinessDisplay 
          avatarImg={<img className="w-full" src={SellerLogo} alt="" />}
          avatarSize="w-[30%] md:w-[50%]"
          title={businessName || "Unknown Seller"}
          className="no-underline"
          StoreInfoClass="w-full grid grid-cols-1 lg:grid-cols-2 gap-y-10 lg:gap-y-0 bg-[#FBFBFB] macScreens:px-10 lg:px-10 py-8"
          verified={isVerified}
          mainIcon={<i className='bx bx-envelope'></i>}
          SellerEmail={email || "N/A"}
          mainIcon1={<i className='bx bx-phone' ></i>}
          SellerContact={phoneNumber || "N/A"}
          mainIcon2={
          ...
          DayMonth={new Date(createdAt).toLocaleDateString() || "N/A"}
          SocialLinksText={businessName}
          ...
          active={true}
          ActionButton={
            <CustomButton buttonText="Start Chat" btnClassName="border-[1px] border-[#00753E] text-[#00753E] hover:bg-transparent flex items-center lg:pe-10 gap-3 relative" icon={<i className='bx bx-message-alt-detail absolute right-5'></i>}/>
          }
          socialLinks={[
        ...
        />
        <div className="">
          <ProfileAds data={products} className="border-b-0" HeaderText={`Products by ${businessName}`} HeaderTextclassName="text-[#161616] font-semibold lg:text-lg windowScreen:text-xl" showComponent={true} showClearButton={false} />
        </div>
      </div>
    </section>
  );
